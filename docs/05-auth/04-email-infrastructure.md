...
The subsystem the v0.1 proposal missed entirely ([critical review §2.3 and §3](../00-foundation/03-critical-review.md)): auth is dead without deliverable email. Every flow in [flows](03-flows.md) that matters — signup verification, password recovery, email change, security notifications — terminates in an inbox. If those mails land in spam, Steadhold Auth is broken *from the customer's point of view* regardless of how correct the token protocol is; and because V1 sends from **shared** Steadhold infrastructure, one abusive project can poison deliverability for every project. This doc specifies the V1 sending architecture, the DNS/authentication setup, the abuse containment, bounce handling, templates, and monitoring.
...
- **From**: `Steadhold Auth <auth@mail.steadhold.app>` — with the project's name in the friendly-from and templates, e.g. `"Acme (via Steadhold)"`, so recipients see whose app is talking. `Reply-To` optionally per-project config.
- **`mail.steadhold.app` is a dedicated sending subdomain**: its reputation is isolated from `steadhold.dev` (corporate mail) and `steadhold.app` (data-plane API hosts). If auth-mail reputation is damaged, the company domain is not.
...
### DNS & authentication for `mail.steadhold.app`

All records managed in Terraform via the Cloudflare provider (D-022, D-023):

| Record | Value (Postmark) | Purpose |
|---|---|---|
| `TXT mail.steadhold.app` | `v=spf1 include:spf.mtasv.net -all` | SPF: only Postmark's MTAs may send as this domain; `-all` hard-fails everything else |
| `CNAME <selector>._domainkey.mail.steadhold.app` | provider-issued DKIM key | DKIM: cryptographic signature over headers/body; the primary reputation identity |
| `TXT _dmarc.mail.steadhold.app` | `v=DMARC1; p=quarantine; rua=mailto:dmarc@steadhold.dev; adkim=s; aspf=s` | DMARC: alignment enforcement + aggregate reports; move `p=quarantine` → `p=reject` after 30 clean days |
| `CNAME pm-bounces.mail.steadhold.app` | provider return-path host | Aligned custom Return-Path so SPF alignment passes |

Also: MX + real mailbox (or route) for `auth@mail.steadhold.app` — a sending address that hard-bounces replies looks like a spammer; route replies to support tooling. Warm-up plan: start on Postmark shared IPs (correct at low volume); dedicated IP only when sustained volume exceeds ~50k/month (OQ-116).
...
Threat: an attacker creates a free project and uses `/signup`//`/recover` as a free bulk-mailer (or to "mail-bomb" a victim address), burning `mail.steadhold.app`'s reputation for everyone. Controls, layered:
...
**Content is fixed templates only in V1**: projects can override template *text* (subject and body copy) but the output is rendered from Steadhold-controlled layouts with **variable interpolation only — no arbitrary HTML, no arbitrary links**. Every URL in an auth mail is constructed by Steadhold from the project's validated `site_url`/redirect allowlist. This kills the phishing kit use-case (attacker cannot make Steadhold send arbitrary content from a reputable domain). Full HTML freedom arrives only with custom SMTP (V1.1), where the sender reputation at stake is the project's own.
...
Provider webhooks → `POST api.steadhold.dev/v1/webhooks/email` (signature-verified) → normalized events:

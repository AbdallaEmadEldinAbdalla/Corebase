...
- **Heartbeats:** long jobs (provision, backup, restore) update `provisioning_jobs.heartbeat_at` every 10 s. A `running` row is orphaned once its heartbeat is older than **three intervals (30 s)** — the threshold is *derived* from the interval, never chosen separately (**D-193**). Steps carry their own bounded timeouts (health wait 5 m, backup 60 m) so a hung step becomes a failed attempt, not a zombie.

  *(The original figures here — a 30 s beat and a 10-minute orphan window, i.e. twenty missed beats — are superseded. Twenty beats of tolerance means a crashed provision stalls for ten minutes, and T6 showed a subtler failure too: when the orphan window outlives the delivery layer's own re-delivery interval, the re-delivery arrives while the row still looks healthy, the worker declines the claim, the delivery is marked complete, and the job is stuck **permanently**. A heartbeat is one `UPDATE`; making it frequent and the threshold a small multiple of it is better on both axes.)*
...
- **D-193 — The orphan threshold is three missed heartbeats (10 s beat → 30 s threshold), derived from the interval rather than chosen independently; a runner constructed with a threshold under two intervals throws.** *(Rationale: the two numbers have a correctness relationship, and separating them lets it drift. Too long and a crashed job's re-delivery is refused by `claim` because the row does not yet look stale — the delivery layer then marks that delivery complete and nothing retries, which T6 reproduced as a project stuck forever. Too short and a live worker loses a job mid-step. A constructor that refuses a bad value is cheaper than a runbook nobody reads.)*

- **D-194 — Sweeper recovery deliveries use their own attempt-keyed delivery id (`<idempotency_key>#recover-<attempt>`), and only when the plain key is already occupied by a dead worker's delivery.** *(Rationale: BullMQ keeps a dead worker's job `active` with a live lock for the whole `lockDuration`, so the sweeper can neither add under that key nor remove it — recovery took a full 60 s lock timeout when Postgres had known the worker was dead for 30. Attempt-keying keeps repeated sweeps idempotent while distinguishing a second crash from the first. This is only safe because the `claim` UPDATE is the mutex: a stale delivery and a recovery delivery racing means one runs and one is skipped. A row that was never delivered keeps the plain key — that is a first delivery, not a recovery, and D-067's `jobId = idempotency_key` still holds for it.)*

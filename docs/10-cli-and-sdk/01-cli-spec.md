...
The full command surface of the `corebase` CLI (proposal §45, §91): authentication against the control plane, project scaffolding and linking, migrations, local development, secrets, logs — and `corebase export`, the D-004 portability flagship. This doc is the contract for the CLI package; the [local development](02-local-development.md) doc owns everything behind `corebase dev`, and [migrations](../03-database-platform/04-migrations.md) owns the exact `db push/pull/reset` semantics (summarized here, never contradicted).

## Design

### Distribution & implementation

Per **D-026**: TypeScript, lives in the monorepo (D-010), distributed via npm — `npm install -g corebase` — with standalone binaries later. Implementation choices (D-135):
...
- All control-plane calls go to the [platform API](../02-control-plane/02-platform-api.md) (`api.corebase.com/v1`) authenticated with a PAT (`Authorization: Bearer cbp_...`, D-062); the CLI sends `Idempotency-Key` on every mutating call it generates.

### Credential storage (D-135)

`corebase login` stores the PAT in the **OS keychain** (macOS Keychain, Windows Credential Manager, libsecret on Linux) via a keytar-equivalent native binding, under service `corebase-cli`, account = the platform user's email. Where no keychain is available (headless Linux, CI containers, WSL without a secret service):

- fallback file `~/.corebase/credentials` (TOML), created with mode **0600** and `~/.corebase/` with 0700;
...
- `COREBASE_ACCESS_TOKEN` env var overrides both (the CI path — no file ever written).

### Configuration precedence

Highest wins: **command-line flags → `COREBASE_*` environment variables → `corebase/config.toml` → built-in defaults.** Env var names are the flag name upper-snake-cased with the prefix (`--project` → `COREBASE_PROJECT`; `COREBASE_ACCESS_TOKEN`, `COREBASE_API_URL` for self-host/staging).

### Project directory & `config.toml`

`corebase init` scaffolds:

```
corebase/
├── config.toml
├── migrations/          # timestamped .sql files (D-028)
└── seed.sql             # applied by `corebase dev` after migrations; never applied to remote
```

```toml
# corebase/config.toml
[project]
# Set by `corebase link`; empty until linked.
ref = "kxqwrtplmzensfba"

[local]
# Ports the `corebase dev` stack binds on localhost (see 02-local-development.md).
...
[dev]
seed = true              # apply seed.sql on `corebase dev` start/reset
```
...
```
corebase login [--token <pat>] [--no-browser]
```

**Behavior:** OAuth-style **device-code flow** against the control plane: the CLI calls `POST /v1/auth/device/code`, prints a short user code and verification URL (`app.corebase.com/cli/verify`), opens the browser (unless `--no-browser`), and polls `POST /v1/auth/device/token` (5s interval, honoring `slow_down`) until the user approves in the dashboard. The control plane mints a PAT scoped `write` (D-062) named `cli-<hostname>`; the CLI stores it per D-135 and prints the account it is logged in as. `--token` skips the flow and stores a pre-created PAT (the CI escape hatch, though `COREBASE_ACCESS_TOKEN` is preferred there).
...
**Behavior:** scaffolds the `corebase/` directory above in the current working directory. Idempotent: existing files are never overwritten without `--force`; missing pieces are added. Does not require login. **Failure modes:** `corebase/` exists with unrecognized layout → exit 5 listing conflicts; no write permission → exit 1.

#### `corebase link`

```
corebase link --project <ref>
```

**Behavior:** verifies the ref exists and the PAT can access it (`GET /v1/projects/:ref`), then writes `project.ref` into `config.toml`. Prints the project name, org, and region as confirmation. **Failure modes:** not logged in → exit 3; ref not found / no access → exit 4 (the platform API deliberately doesn't distinguish, no existence oracle); no `corebase/` dir → exit 4 with `corebase init` hint.
...
#### `corebase dev` (and `dev stop`, `dev destroy`, `--db-only`)
...
#### `corebase migration new <name>`

```
corebase migration new add_profiles
```

**Behavior:** creates `corebase/migrations/<UTC timestamp>_add_profiles.sql` (e.g. `20260827130000_add_profiles.sql`, §43) with a comment header. `migration create` is accepted as an alias for §43 muscle memory. **Failure modes:** name not `[a-z0-9_]+` → exit 2; duplicate timestamp collision → increments one second.

#### `corebase export` — the D-004 flagship

```
corebase export [--output <path>] [--include-objects] [--no-data]
```

**Behavior:** produces a complete, self-describing portability snapshot of the linked project as a tarball. Everything needed to stand the project up elsewhere — another Corebase, a self-hosted stack, or bare Postgres. Layout (D-137, `format_version: 1`):

```
corebase-export-<ref>-<UTC timestamp>.tar.gz
...
│   └── REIMPORT.md             # hash format + how to load into another Corebase or your own auth
├── storage/
...
#### `corebase projects create | list | delete`

```
corebase projects create --name <name> --org <org_id> [--region eu-central] [--plan free]
corebase projects list [--org <org_id>]
corebase projects delete <ref>
```
...
#### `corebase secrets set | list | unset`

```
corebase secrets set NAME=value [NAME2=value2 ...] [--env-file <path>]
corebase secrets list
corebase secrets unset NAME [NAME2 ...]
```
...
```
corebase logs [--service api|auth|db] [--tail] [--since 1h]
```
...
| 4 | Not found / not linked (project ref, missing `corebase/` dir) |
...
- **Update check:** at most once per 24h, the CLI checks the npm registry for a newer version (cached in `~/.corebase/update-check.json`) and prints a one-line notice to **stderr**. Disabled by `COREBASE_NO_UPDATE_CHECK=1`, in CI (`CI=1`), and in `--json` mode. The check sends nothing but the standard npm registry request.
- **Telemetry: opt-in only.** No usage data, no crash reports, no identifiers leave the machine unless the user runs `corebase telemetry on` (and `telemetry status` / `off` exist). Default-off is a portability-lane trust signal (D-006) and one less thing to disclose. When enabled: anonymous command name + duration + version, no arguments, no project refs.

## Decisions

- **D-135 — CLI is built on commander (Node ≥ 20); PATs are stored in the OS keychain via a keytar-equivalent, with fallback `~/.corebase/credentials` created 0600 and refused if group/world-readable; `COREBASE_ACCESS_TOKEN` overrides both.** *(Rationale: commander is the boring, universally-understood choice for a flat command tree; keychain-first keeps tokens out of dotfiles, the perms-checked fallback and env override cover headless and CI without weakening the default.)*
- **D-136 — Telemetry is opt-in only (default off, `corebase telemetry on` to enable); the update check is a 24h-cached npm registry poll, disabled by env var, in CI, and in `--json` mode.** *(Rationale: a portability-pitched tool that phones home by default undercuts its own story; the update nudge is the one growth mechanism cheap enough to keep without collecting anything.)*
- **D-137 — `corebase export` emits a versioned tarball (`format_version: 1`) with the documented layout: `export.json` manifest with checksums, `database/schema.sql` + `database/data.dump` (pg_dump plain + custom), `migrations/`, `auth/users.jsonl` including password hashes plus `REIMPORT.md`, `storage/manifest.jsonl` (+ objects opt-in), `config/project.json` with secret names only.** *(Rationale: D-004 says portability is the moat — a moat needs a stable, documented artifact format, not a directory of ad-hoc dumps; hashes-with-warning makes exports actually complete, and versioning the format lets it evolve without breaking existing tarballs.)*

## Open Questions

- **OQ-135** — `corebase import` (the re-import counterpart to export, targeting another Corebase project or a self-hosted stack): V1.1 candidate or documented-manual-procedure-only? `REIMPORT.md` covers manual paths for now.
...
- **Feeds:** [local development](02-local-development.md) (`corebase dev`) · [SDK spec](03-sdk-spec.md) (`gen types`) · [open-source strategy](../12-business/04-open-source-strategy.md) (CLI is Apache-2.0, D-034) · [testing strategy](../13-quality/01-testing-strategy.md) (CLI in CI) · [open questions](../15-risks/02-open-questions.md)

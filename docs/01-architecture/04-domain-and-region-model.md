...
|---|---|---|---|
| `corebase.com` | — | marketing site (Cloudflare Pages) | no app cookies here, ever |
| `app.corebase.com` | control | dashboard (Next.js) | dashboard session cookies scoped to this host |
| `api.corebase.com` | control | platform REST API `/v1` (D-039) | CLI + dashboard talk here |
| `<ref>.corebase.co` | data | customer traffic: `/rest/v1`, `/auth/v1`, `/storage/v1` | each project its own origin → browser same-origin policy separates tenants for free |
| `db.<ref>.corebase.co` *(reserved)* | data | direct Postgres / pooler TCP endpoint when we later front 5432/6432 via load balancer; V1 hands out node-addressed connection strings | |

Why two apexes: customer projects live on **`.corebase.co`** so that no customer-controlled content or credential ever shares a registrable domain with the dashboard (`app.corebase.com`). A tenant XSS or cookie-scope bug on a project subdomain cannot touch dashboard sessions, and the Public Suffix List becomes an option for `corebase.co` later. Proposal §61's `<ref>.storage.corebase.co` is **dropped in V1**: a wildcard certificate matches exactly one label, so `*.corebase.co` does not cover `x.storage.corebase.co` — a second wildcard cert and routing tier for zero V1 benefit. Storage rides the project origin at `/storage/v1/*` instead.
...
example:  kf3m7q2xvbn4w6yc5zta  →  kf3m7q2xvbn4w6yc5zta.corebase.co
```
...
client ──TLS 1───▶ Cloudflare edge ──TLS 2───▶ Caddy (app node) ──loopback──▶ gateway
         *.corebase.co wildcard        Cloudflare Origin CA cert,
         + corebase.com/app/api        Full (strict) + authenticated
         certs, auto-managed           origin pulls (mTLS)
```

- **TLS 1 (client → edge)**: Cloudflare holds and auto-renews the certificates — apex + `*.corebase.com`, apex + `*.corebase.co` (wildcard TLS at the edge is half of D-016). Every project subdomain is covered by the one wildcard; SNI from the client selects it, nothing per-project exists.
- **TLS 2 (edge → origin)**: Cloudflare **Full (strict)** mode to Caddy, which serves a long-lived **Cloudflare Origin CA** certificate for `*.corebase.co` and the `corebase.com` hosts (D-050 in [system architecture](01-system-architecture.md)). **Authenticated origin pulls** (Cloudflare's client cert, verified by Caddy) plus a firewall allowlist of Cloudflare IP ranges mean the origin refuses everything that didn't come through the edge — otherwise the wildcard DNS would let attackers hit Caddy directly and skip edge rate limiting/DDoS.
- **Why no per-project certificates in V1 (D-057)**: per-project certs exist to serve **custom domains** (`api.customer.com`) — a V1 non-feature. For `<ref>.corebase.co` the wildcard is strictly better: zero issuance latency at provision time (a Let's Encrypt round-trip per project would add seconds-to-minutes and a rate-limit ceiling), zero renewal fleet to babysit, zero cert-per-tenant storage. Isolation is not weakened: TLS identifies the *platform*, while tenant separation is enforced by routing + keys + RLS. Custom domains, when they come (V1.x), use Cloudflare for SaaS per-hostname certs — additive, no rearchitecture (OQ-057).
...
```text
Host: kf3m7q2xvbn4w6yc5zta.corebase.co
  1. exactly one label before corebase.co?  else 404
...
Caddy is configured with exactly two virtual hosts (`*.corebase.co` → gateway, control-plane hosts → their modules); it never knows about individual projects. All tenant knowledge lives in one place — the routing table (D-051) — so there is no per-project state to keep synchronized across a proxy fleet.
...
- **OQ-062** — Should `corebase.co` be submitted to the Public Suffix List once custom subdomain content (e.g. storage-served HTML) exists, to hard-isolate tenant cookies/localStorage at the browser level? Has side effects on any future shared-cookie feature; owner: [threat model](../06-security/01-threat-model.md).

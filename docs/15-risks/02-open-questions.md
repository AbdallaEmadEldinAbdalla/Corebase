...
| OQ-184 | Is the request-pipeline budget's ~1.5 ms gateway overhead achievable, or does the figure want revising? Measured at 2.91 ms per request with Redis round-trips Docker-forwarded and the three rate-limit layers deliberately sequential (D-389) | [request pipeline](../04-data-api/02-request-pipeline.md) | needs a production-shaped node; before the SLO is advertised |

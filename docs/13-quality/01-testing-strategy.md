...
- **OQ-151 — k6 threshold numbers:** the request-pipeline thresholds are owned by the latency budget in [request pipeline](../04-data-api/02-request-pipeline.md); until that doc locks numbers, the k6 suite runs in record-only mode. Decide the lock date alongside that doc.

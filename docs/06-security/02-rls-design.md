...
4. **Direct connections skip the pipeline** — a customer psql session is whatever role they logged in as. RLS still applies to their non-privileged roles because of `FORCE ROW LEVEL SECURITY` (below), but the customer owner role can alter policies; that is by design (it's their database). Isolation from *other* tenants never depends on RLS — that's the container boundary (D-009, D-081).
...
- `GRANT` still gates above RLS (D-108, normative DDL in [api-keys-and-roles](../04-data-api/03-api-keys-and-roles.md)): default privileges grant table-level `SELECT/INSERT/UPDATE/DELETE` to `authenticated` and `service_role` only; **`anon` gets no default table grants** — anonymous access is opt-in via explicit per-table `GRANT` (the dashboard toggle emits it). RLS then filters rows for the roles that do hold grants, and force-RLS at creation (D-083) keeps the whole arrangement fail-closed. Revoking a grant entirely is the coarse switch for "this table is never API-visible."

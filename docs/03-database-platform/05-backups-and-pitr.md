...
`archive-push` runs asynchronously with a small spool (`archive-async=y`, spool on the project volume — inside the disk quota, so a runaway spool is the tenant's ceiling, not the node's). That async worker holds pgBackRest's archive lock while it batches, so **every other pgBackRest command must retry through lock contention** rather than fail (**D-267**): the busier a project is, the likelier its backup commands lose that race. **Archiving failure is a first-class alert**: WAL-archive lag alerts at **>5 min (warn)** and **>15 min (page)** per the [observability](../11-infrastructure/03-observability.md) alert catalog, where lag is **the age of the oldest segment closed but not yet archived** and not the time since the last successful push (**D-271** — the naive reading pegs every healthy idle Free project at the warn line, because `archive_timeout` closes a segment every 300 s and nothing happens in between); the `pgbackrest check` job runs every 15 min per project, and **on any sweep where the project is past the warn rung, its archiver failure count moved, or its last check failed** (**D-273**) — the interval is a cost control, not a policy, and a check that stays stale after recovery keeps the pager ringing for a quarter of an hour after the fix. It feeds that same alert rather than owning a separate threshold — a project whose WAL isn't landing in R2 is a project whose PITR is silently rotting.
...
Scheduling is a control-plane job per project (BullMQ, D-018) — *not* per-node cron — so the scheduler can jitter backups across the node fleet, skip paused projects (§7), and record every run's outcome in `backup_runs(project_id, type, started_at, finished_at, size_bytes, wal_start/stop, status, error)`. The table records **attempts**, not only the backups that exist (**D-278**): a project whose nightly full has failed for six days is indistinguishable through `pgbackrest info` from one whose retention window starts six days ago, and failures leave no trace in a repo by definition. A project's slot inside the window is derived from its id rather than random (**D-280**), the job's idempotency key carries the calendar day rather than a timestamp (**D-279**), and a project that has never been backed up — or is more than twice its interval overdue — is backed up immediately, window or no window (**D-281**).
...
Notes: retention is `repo1-retention-full-type=time` — **days**, not a count of backups (**D-276**; the two coincide only on Free, where fulls are nightly, and `count` on Pro's weekly fulls would keep about eight months of them against a 30-day promise). Time also gives the guarantee a count cannot: pgBackRest expires a full older than the window only while another at least that old remains, so a base always exists *before* the oldest restorable point — which is what the slack in Pro's 35-against-30 buys. WAL expiry follows full-backup expiry in pgBackRest, so the retention-full window is the PITR window; R2's zero egress (D-017) means retention cost is storage-only, which is why even Free gets true PITR — it is a differentiation-lane feature (D-006) that costs cents. Deleted projects: deletion takes a **final full backup and stops if it fails** (**D-296**, **D-299** — the gate is on by default), because nothing about a `DELETE` distinguishes "we are done with this" from "I typed the wrong ref", and a recovery window with nothing behind it looks exactly like a recovery window until someone needs it. The 7-day recovery window (D-038) is the pre-purge recovery period; the restore-verified final backup is kept **30 days past purge** (D-066), and only after purge + 30 d is the repo path destroyed, with the destruction recorded in the audit log. The **control plane** does that deletion itself over S3 (**D-301**): by then the container, volume, placement row and credentials are all gone, so there is nowhere left to run `pgbackrest` — and delete rights belong to the control plane alone in any case (§6). What must survive the purge is the schedule, which lives in `project_repos` (**D-302**). Destruction is **proven** by re-listing the prefix and is only recorded when that list comes back empty (**D-303**); a failure leaves the repo due, so every sweep retries and the stored error makes a repo that cannot be destroyed visible.
...
2. **Restore**: `pgbackrest --stanza=main restore --type=time --target='2026-08-27 14:03:00+00' --target-action=pause` into the new volume, run by a throwaway container that starts no database — the image's entrypoint would `initdb` the volume the restore is about to fill. pgBackRest picks the correct base backup + chain automatically, and needs one whose *stop time* is strictly before the target: a target inside the same second as a backup's completion is unrestorable. The **source's** repo config is then written into the real project container before Postgres starts (**D-284**) — `restore_command` runs `archive-get` inside that container for every segment recovery wants, so the copy's own config would point it at an empty repo.
3. **Recover**: start Postgres; it replays WAL to the target; with `--target-action=pause` the instance holds at the target for inspection; the restore job then promotes it (`pg_wal_replay_resume`) once replay confirms the target was reached. **`pause` is what makes that confirmation possible at all** (**D-285**): `pg_get_wal_replay_pause_state() = 'paused'` while still in recovery can only happen if the target was understood *and* reached, because a cluster that ignored the settings or ran out of WAL would have finished recovery and promoted itself. Under `promote` there is no such moment, and a restore that ran short would arrive as a healthy database holding the wrong day. Failure to reach the target fails the job loudly — never silently serve an earlier point. Evidence is recorded as `reached_lsn`; `reached_time` is informational and may be NULL (**D-286**).
...
7. If not promoted, the restored instance **auto-expires after 48 h** — configurable, but never beyond **7 days** (**D-291**: the ceiling is not a setting, because a too-long TTL fails silently and cumulatively). Expiry hands the copy to the *normal deletion pipeline* (**D-292**), so it is soft-deleted with a final backup and its data stays recoverable for the usual window — the deadline is when the copy stops running, not when the data is destroyed. That distinction matters more here than anywhere: the customer restored because they lost data, so the copy may be the only surviving version of something. The deadline is set in the same transaction as the project and appears on the project detail, not only in the reply to the create (**D-294**). It bills as a temporary instance on paid plans; one free restore-validate at a time on Free.
...
- **On pause** (step 2 of the [pause procedure](01-postgres-provisioning.md)): take a final backup — incremental if the chain is fresh (< 7 d since last base), else full (**D-298**) — after the final checkpoint, and confirm the last WAL segment archived (`pgbackrest check`) **before** the container stops. Pause does not complete until the project is restorable from R2 alone, and a failure **leaves the containers running** rather than pausing anyway (**D-297**): a paused project whose backup failed has one node disk as the only copy of the data, which is strictly worse than a running project and the opposite of what pausing is for. The order is not incidental — a backup taken before the checkpoint omits exactly what the checkpoint flushed. A project pausing before it has ever had a full takes one, because an incremental with no base is not a thing.

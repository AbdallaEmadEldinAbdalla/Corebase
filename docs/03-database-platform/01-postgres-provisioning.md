...
- **Bin-packing**: new placements go to the active node in the region with the **lowest RAM fill ratio** that still has `ram_reserved + request ≤ 0.85 × ram_total` and disk headroom (spread-first — favors resume headroom and blast-radius over max density; revisit toward fill-first when node count grows). Warm-pool stacks are booked like projects so they are part of the same arithmetic.

...
describe('T7 — soft delete keeps the data', () => {
  t('stops the container, keeps the volume, and opens a recovery window', async () => {
    ...
    // Six passwords (the pooler's joined in P2b, the auth module's in P4a, the
    // dashboard console's in P7l) + keypair (3 rows) + two minted keys: all kept,
    // because a soft delete destroys nothing.
    expect(after.secrets).toBe(11);

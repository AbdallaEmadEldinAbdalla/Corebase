 ...
 *   SH_NR_COUNT=3 pnpm --filter @steadhold/worker node-reboot
 */
...
const COUNT = Number(process.env.SH_NR_COUNT ?? 5);
const PORT = Number(process.env.SH_NR_API_PORT ?? 8094);
const TOKEN = 'nr-token-harness-token-long-enough-for-the-boot-check';
const CONVERGE_MS = Number(process.env.SH_NR_CONVERGE_MS ?? 180_000);
...
const NODE_CONTAINER = process.env.SH_NR_NODE_CONTAINER ?? 'sh-data-node';
...
const env = {
  ...
  SH_BOOTSTRAP_SECRET: process.env.SH_BOOTSTRAP_SECRET ?? 'nr-bootstrap-secret-0123456789',
  ...
  SH_METRICS_PORT: process.env.SH_METRICS_PORT ?? '9113',
  ...
  SH_RECONCILE_INTERVAL_MS: process.env.SH_RECONCILE_INTERVAL_MS ?? '5000',
};
...
async function main() {
  ...
  for (const c of await containers()) {
    if (c.Names.some((n) => n.startsWith('/sh-'))) {
  ...
  for (const v of stale.Volumes ?? []) {
    if (v.Name.startsWith('sh-')) await nodeApi(`/volumes/${v.Name}`, 'DELETE').catch(() => {});
  }
  ...
  for (let i = 1; i <= COUNT; i++) {
    const res = await fetch(`http://127.0.0.1:${PORT}/v1/projects`, {
      ...
      body: JSON.stringify({
        name: `nr-${process.pid}-${i}`, region: 'eu-central', org_id: await orgId() }),
    });
  ...
  const brokenContainer = (await containers()).find((c) => c.Names.includes(`/sh-${brokenRef}`))!;
  ...
  await waitFor('projects that had a restart policy came back on their own', async () => {
    ...
    return survivors.every((r) =>
      live.find((c) => c.Names.includes(`/sh-${r}`))?.State === 'running');
  }, 120_000);
  ...
  const stillThere = (await containers()).some((c) => c.Names.includes(`/sh-${orphanRef}`));
